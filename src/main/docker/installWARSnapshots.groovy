...
static void installPluginSnapshots(File directory, File explodeDir, File mavenSettings = null, String javaOpts="") throws IOException {
    ...
    for (File plugin : plugins) {
        if (plugin.getAbsolutePath().endsWith(".hpi")) {
            ...
            installHPILocally(plugin, pluginExplodeDir, mavenSettings, javaOpts)
        }
    }
}

static void installHPILocally(File plugin, File tmpDir, File mavenSettings = null, String javaOpts="") {
...
static void installJARLocally(File jar, File tmpDir, File mavenSettings = null, String javaOpts="") {
    def pluginData = readPluginManifest(jar)
    if (!pluginData.version.contains("SNAPSHOT")) {
        return
    }
    println "Installing SNAPSHOT for ${pluginData.artifactId}:${pluginData.version}"
    execOrFail("unzip -q ${jar.absolutePath} -d ${tmpDir.absolutePath}");
...
static ArtifactData readPluginManifest(File sourceHPI) throws IOException {
...
static ArtifactData readJarManifest(File sourceJAR) throws IOException {
    def zipFile = new ZipFile(sourceJAR)
    InputStream is
    try {
        def entries = zipFile.entries()
        while (entries.hasMoreElements()) {
